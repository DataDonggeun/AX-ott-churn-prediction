...
def _align_X(model, X: pd.DataFrame) -> pd.DataFrame:
    """저장된 모델이 학습 시 기억한 피처 순서에 맞춰 X 컬럼을 재정렬한다.

    sklearn 계열은 feature_names_in_, CatBoost는 feature_names_ 를 노출한다.
    피처 집합·순서가 어긋나면 predict_proba 가 거부하므로(특히 재생성된
    expanded_dataset 의 컬럼 순서가 달라졌을 때) 모델 기준으로 정렬한다.
    누락 컬럼은 0으로 채운다.
    """
    names = getattr(model, "feature_names_in_", None)
    if names is None:
        fn = getattr(model, "feature_names_", None)  # CatBoost
        names = list(fn) if fn else None
    if names is None or len(names) == 0:
        return X
    names = list(names)
    X2 = X
    missing = [c for c in names if c not in X2.columns]
    if missing:
        X2 = X2.copy()
        for c in missing:
            X2[c] = 0
    ...
    for scope_name, scope_fn in SCOPES.items():
        ...
        if adopted:
            ...
            for name in col_names:
                ...
                if m is not None:
                    base_preds.append(m.predict_proba(_align_X(m, X))[:, 1])
            ...
            if base_preds and meta_clf is not None:
                ...
                scores    = cb.predict_proba(_align_X(cb, X))[:, 1] if cb is not None else np.zeros(len(X))
                model_src = "catboost_fallback"
        else:
            cb = load_artifact(f"tuned_model_{scope_name}")
            if cb is not None:
                scores    = cb.predict_proba(_align_X(cb, X))[:, 1]
