"""
Step 10: 세그멘테이션 설계 (Segmentation Design)

역할: FastAPI
목적: Step 08의 payment-removed OOF churn_risk 점수와
      day0~20 행동 규칙을 결합해 6개 대표 세그먼트를 배정한다.
      payment/auth/demographic proxy는 규칙으로 사용하지 않음.
출력: 세그먼트별 행 수·재구매율, 전체 배정 CSV
캐시: step10_segment_summary.json, step10_segment_assignment.csv
"""
import sys
...
router = APIRouter(prefix="/10", tags=["10. Segmentation"])
...
def run_segmentation(exp_df: pd.DataFrame, oof_df: pd.DataFrame) -> dict:
    ...
    save_df("step10_segment_assignment",
...
def segmentation(force: bool = False):
    ...
    if oof_df is None:
        return {"status": "FAIL", "reason": "Step 08 먼저 실행 필요"}

    result = run_segmentation(exp_df, oof_df)

    save_json("step10_segment_summary", result)
    mark_done("step10", {
...
def segment_summary():
    ...
    if cached:
        cached["from_cache"] = True
        return cached
    return {"status": "NOT_READY", "message": "Step 10 아직 실행 안 됨"}
